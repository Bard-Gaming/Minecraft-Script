...
class CompileInterpreter:
    ...
    def visit_DefineFunctionNode(self, node, context: CompileContext) -> CompileResult:
        ...
        self.add_command(fnc_name, "# File generated by Minecraft-Script")  # add comment to add function to .get_mcs_functions()
        # Don't generate function yet; generate them all at the end
...
def mcs_compile(ast, functions_dir: str, datapack_id):
    ...
    interpreter.visit(ast, context)  # create everything except function bodies
    for fnc_name in interpreter.get_mcs_functions():
        context.get(fnc_name).generate_function(interpreter)
